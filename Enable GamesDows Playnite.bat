...
:: Define Paths - Quotes used to handle spaces in usernames
SET "KEY_NAME=HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon"
SET "VALUE_NAME=Shell"
:: Attempt to locate Playnite. If %LOCALAPPDATA% is wrong due to admin elevation, 
:: you may need to hardcode your path below, e.g. SET "PLAYNITE_FOLDER=C:\Users\YourName\AppData\Local\Playnite"
...
:: Verify Playnite exists before proceeding
IF NOT EXIST "%PLAYNITE_PATH%" (
    echo.
    echo ERROR: Could not find Playnite at:
    echo "%PLAYNITE_PATH%"
    echo.
    echo If you are running this as a different Admin user, variables may be wrong.
...
echo.
echo :: Set Shell to Explorer BEFORE launching it (critical)
echo REG ADD "HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" /v Shell /t REG_SZ /d "C:\Windows\explorer.exe" /f
echo.
...
:: /rl highest : Runs with Admin privileges (Required to write to Registry in the bat)
:: /tr : The command to run. NOTE: We use escaped quotes \" around the path to handle spaces in AppData.
